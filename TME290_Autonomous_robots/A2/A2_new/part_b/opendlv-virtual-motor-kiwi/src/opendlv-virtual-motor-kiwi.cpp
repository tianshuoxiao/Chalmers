...
int32_t main(int32_t argc, char **argv) {
  ...
  if (0 == commandlineArguments.count("cid") 
    ...
    auto onAngleLeftRequest{[&singleTrackModel](
        cluon::data::Envelope &&envelope) // , &INPUT_ID
      {
        uint32_t const senderStamp = envelope.senderStamp();
        if (senderStamp == 0) {
          auto angleLeftRequest = 
            cluon::extractMessage<opendlv::proxy::AxleAngularVelocityRequest>(
                std::move(envelope));
          singleTrackModel.setLeftAngularVelocity(angleLeftRequest);
        }
  
        else if (senderStamp == 1) {      //
          auto angleRightRequest = 
            cluon::extractMessage<opendlv::proxy::AxleAngularVelocityRequest>(
                std::move(envelope));
          singleTrackModel.setRightAngularVelocity(angleRightRequest);
        }
      }};      
/*      
