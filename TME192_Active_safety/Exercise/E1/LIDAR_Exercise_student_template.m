...
for iIndex=1:length(oData.iTimeSF)
   ...
   x_Lidar = oData.fLIDAR_X{1, iLIDARIndex};
   y_Lidar = oData.fLIDAR_Y{1, iLIDARIndex};
   x_LidartoGPS = x_Lidar+oData.fLIDARposX-oData.fGPSposX;
   y_LidartoGPS = y_Lidar+oData.fLIDARposY;
   
   theta = (asin(y_LidartoGPS.*sqrt(x_LidartoGPS.^2+y_LidartoGPS.^2).^-1)+oData.fHeadingSF(iIndex));
